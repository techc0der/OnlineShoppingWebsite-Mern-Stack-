...
const Showproduct = ({ editproduct, seteditproduct }) => {
  ...
  const filterSubmit = (e) => {
    ...
    fetch(`http://localhost:3000/product/filter/seller?${params.toString()}`, {
      method: 'POST',
      headers: {
  ...
  const ondelete = async (productid) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/product/seller/delete?${deleteproduct.toString()}`, {
        method: 'DELETE',
        headers: {
